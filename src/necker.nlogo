...
This is a "constraint satisfaction" neural network model written in NetLogo (a tool for writing computer simulations).  This is a simple model of a perceptual process for interpreting a 2-D image known as a Necker cube.  Humans normally perceive Necker cube images as if they were three-dimensional, but there are two different ways of doing this (https://en.wikipedia.org/wiki/Necker_cube).  Some people perceive the Necker cube is perceived one way, sometimes the other way, and sometimes a person's perception can "flip" between the two ways of seeing it.  Some background discussion will help to clarify this idea.
...
More specifically, there are two Necker cube figures in this model. One represents a perception that one is looking at the cube from below, and the other represent a view from above.  The little circles represent "hypotheses" about the locations of corners of a 3D figure in space.  These have colors ranging from -1 (red, "no, the hypothesis is incorrect") to 1 (blue, "yes, the hypothesis is correct").  Black lines between these "nodes" within each Necker cube figure represent positive "constraints" that tend to force connected nodes to agree on their "yes" or "no" answers.  (That is, the black lines are both the lines in the Necker cube figure, *and* they represent neural network links between nodes.)  Red lines between the two Necker cube figures tend to force the nodes at either end to have *opposite* values (yes, no; or no, yes).  

When the model runs, the values (-1 to 1) of the nodes are gradually adjusted in responses to the values of their connected neighbor nodes.  Eventually, the entire network should reach a state where all of the corner nodes in one Necker cube figure are blue, indicating that these "corner" hypotheses are all consistent with each other, and the figure is being "perceived" (by the model) from the perspective (from above, from below) represented by that Necker cube figure.  At the same time, all of the nodes in the other Necker cube should be red, indicating that *that* way of perceiving the figure has been rejected.
