...
;; This is not the algorithm in the Rumelhart et al paper, but it works better.
to settle-network
...
;; An algorithim inspired by Rumelhart et al PDP book chapter 14, vol II, p. 11
;; they use sequential update, but I use effectively simultaneous update.
;; Often produces paradoxical results.  My algorithim works better.
to rumelhart-settle-network
  ask nodes [set prev-activation activation] ; do this first to allow updates to be effectively simultaneous

  ask nodes [
    let net-input 0
    ask my-constraints [ ask other-end [set net-input net-input + (prev-activation * [weight] of myself)] ]
    set activation prev-activation + ifelse-value (net-input > 0)
                                       [net-input * (1 - prev-activation)]
                                       [net-input * prev-activation]
    set activation max (list 0 (min (list 1 activation)))
    update-node-color self
  ]
end

to update-node-color [a-node]
  ask a-node [ set color ( rgb (max (list 0 (min (list 255 (255 * (1 - activation))))))
                               (150 * activation)
                               (max (list 0 (min (list 255 (255 * activation))))) ) ]
end
...
to-report create-a-node [x y]
  ...
  hatch-nodes 1 [
    set activation random-float 1        ; Rumelhart et al version from 0 to 1
    ;set activation (random-float 2) - 1 ; my original version ranges from -1 to 1
    update-node-color self
